...
module.exports.run = async (bot, message, args) => {
    ...
    bdd.query(`SELECT * FROM lock WHERE id = '${salon.id}'`, (err, rows) => {
        if(rows.length > 1) return utils.errorPerso(message, "Ce salon ne peut pas être unlock si vous pensez que c'est une erreur contactez Raffon ou Franklin");
    ...
    message.channel.send("<a:loading:566988474436288515> Ouverture du salon").then(m =>
        setTimeout(function(){
           m.edit(`<:confirm:566988762848952321> Salon ouvert avec succès`)
           m.delete(5000)
        }, 2000)        
    )
